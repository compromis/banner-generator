...
Chart.elements.Rectangle.prototype.draw = function () {
  ...
  var cornerRadius = 10
  var fullCornerRadius = false
  var stackedRounded = false
  ...
  for (var i = 1; i < 4; i++) {
    ...
    if (height < 0) {
      ...
      ctx.quadraticCurveTo(x_bl, y_bl, x_bl + radius, y_bl)
        ...
        // bottom right
        if (fullCornerRadius || typeOfChart == 'horizontalBar') { ctx.quadraticCurveTo(x + width, y + height, x + width - radius, y + height) } else { ctx.lineTo(x + width, y + height, x + width - radius, y + height) }

        ctx.lineTo(x + radius, y + height)

        // bottom left
        if (fullCornerRadius) { ctx.quadraticCurveTo(x, y + height, x, y + height - radius) } else { ctx.lineTo(x, y + height, x, y + height - radius) }

        ctx.lineTo(x, y + radius)

        // top left
        if (fullCornerRadius || typeOfChart == 'bar') { ctx.quadraticCurveTo(x, y, x + radius, y) } else { ctx.lineTo(x, y, x + radius, y) }
    }
  }
